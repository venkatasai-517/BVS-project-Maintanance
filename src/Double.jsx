...
function App() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("second").on("value", (snapshot) => {
  ...
  const submitHandler = async (e) => {
    ...
    await firebaseDB.child("second").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`second/${selectedRecord.id}`).update(data1);
  ...
  const handleDeleteButtonClick = (row) => {
    try {
      firebaseDB.child(`second/${row.id}`).remove();
