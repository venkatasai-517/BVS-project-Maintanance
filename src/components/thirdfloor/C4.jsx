...
function Cfour() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("C4").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("C4").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("C4").get();
      ...
      await firebaseDB.child("C4").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`C4/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`C4/${key}`).remove();
