...
function Ctwo() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("C2").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("C2").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("C2").get();
      ...
      await firebaseDB.child("C2").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`C2/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`C2/${key}`).remove();
