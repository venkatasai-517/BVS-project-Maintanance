...
function Cthree() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("C3").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("C3").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("C3").get();
      ...
      await firebaseDB.child("C3").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`C3/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`C3/${key}`).remove();
