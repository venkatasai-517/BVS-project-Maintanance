...
function Afour() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("A4").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("A4").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("A4").get();
      ...
      await firebaseDB.child("A4").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`A4/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`A4/${key}`).remove();
          ...
          <div className="modal-content">
            ...
            <div className="modal-body">
              <form onSubmit={submitHandler}>
                ...
                <div className="col-md-4">
                  <label className="form-label">Tenent_img</label>
                  <input
                    type="file"
                    name="student_img"
