...
function Asix() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("A6").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("A6").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("A6").get();
      ...
      await firebaseDB.child("A6").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`A6/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`A6/${key}`).remove();
