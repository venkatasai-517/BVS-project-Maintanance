...
import { v4 } from "uuid";
import { imgDB } from "../../firebase.jsx";
...
function Aseven() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("A7").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("A7").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("A7").get();
      if (snapshot.exists() && Object.keys(snapshot.val()).length >= 2) {
        alert(
          "You cannot add more than 2 records. because this room contains 2 Beds only so please check the Beds."
        );
        return;
      }
      ...
      await firebaseDB.child("A7").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`A7/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`A7/${key}`).remove();
