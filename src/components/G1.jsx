...
function Gone() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("G1").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("G1").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("G1").get();
      ...
      await firebaseDB.child("G1").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`G1/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`G1/${key}`).remove();
