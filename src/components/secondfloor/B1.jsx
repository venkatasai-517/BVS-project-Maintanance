...
function Bone() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("B1").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("B1").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("B1").get();
      ...
      await firebaseDB.child("B1").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`B1/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`B1/${key}`).remove();
