...
function Bsix() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("B6").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("B6").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("B6").get();
      ...
      await firebaseDB.child("B6").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`B6/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`B6/${key}`).remove();
