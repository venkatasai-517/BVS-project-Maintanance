...
function Bthree() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("B3").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("B3").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("B3").get();
      ...
      await firebaseDB.child("B3").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`B3/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`B3/${key}`).remove();
