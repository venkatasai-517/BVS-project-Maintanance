...
function Bfive() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("B5").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("B5").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("B5").get();
      ...
      await firebaseDB.child("B5").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`B5/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`B5/${key}`).remove();
