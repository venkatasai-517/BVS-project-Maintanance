...
function Gtwo() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("G2").on("value", (details) => {
        if (details.val()) {
    ...
    return () => firebaseDB.child("G2").off();
  }, []);
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const snapshot = await firebaseDB.child("G2").get();
      ...
      await firebaseDB.child("G2").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`G2/${selectedRecord}`).update(updatedData);
  ...
  const handleDeleteButtonClick = (key) => {
    if (window.confirm("Do you want to delete?")) {
      try {
        firebaseDB.child(`G2/${key}`).remove();
