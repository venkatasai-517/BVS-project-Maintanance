...
function App() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("four").on("value", (snapshot) => {
  ...
  const submitHandler = async (e) => {
    ...
    await firebaseDB.child("four").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`four/${selectedRecord.id}`).update(data1);
  ...
  const handleDeleteButtonClick = (row) => {
    try {
      firebaseDB.child(`four/${row.id}`).remove();
