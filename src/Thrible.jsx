...
function App() {
  ...
  useEffect(() => {
    const fetchData = () => {
      firebaseDB.child("thrible").on("value", (snapshot) => {
  ...
  const submitHandler = async (e) => {
    ...
    await firebaseDB.child("thrible").push(formDataWithImgUrl);
  ...
  const submitEditHandler = async (e) => {
    ...
    try {
      ...
      await firebaseDB.child(`thrible/${selectedRecord.id}`).update(data1);
  ...
  const handleDeleteButtonClick = (row) => {
    try {
      firebaseDB.child(`thrible/${row.id}`).remove();
